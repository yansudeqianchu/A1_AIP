...
def checkTrajs(sol, problem):
    ...
    for p in pacman_positions:
        ...
        prob = search.SingleFoodSearchProblem(pos=(x1, y1), food=new_food_grid, walls=walls)

        res = checkSolution(prob, sol[p])
        if not res:
            return False
...
class MAPFTest(testClasses.TestCase):
    ...
    def execute(self, grades, moduleDict, solutionDict):
        ...
        if not checkSolution(problem, sol):
            grades.addMessage('FAIL: %s' % self.path)
...
class HeuristicGrade(testClasses.TestCase):
    ...
    def execute(self, grades, moduleDict, solutionDict):
        search = moduleDict['search']
        problem, _, heuristic = self.setupProblem(search)
